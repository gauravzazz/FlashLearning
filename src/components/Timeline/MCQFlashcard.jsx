...
const MCQFlashcard = ({ question, options, correctAnswer, explanation, cardColor }) => {
  ...
  return (
    <div className="w-80 h-56 m-auto perspective" onClick={() => setIsFlipped(!isFlipped)}>
      <div className={`relative w-full h-full transition-transform duration-700 ease-in-out transform-style preserve-3d shadow-lg rounded-lg ${isFlipped ? 'rotate-y-180' : ''}`}>
        {!isFlipped ? (
          <div className={`absolute w-full h-full flex flex-col justify-center font-bold text-lg rounded-lg text-center p-4 ${cardColor.front}`}>
          ...
          <div className={`absolute w-full h-full flex flex-col items-center justify-center font-bold text-lg rotate-y-180 rounded-lg text-center p-4 ${cardColor.back}`}>
